...
The compare lab now also includes a shared side-by-side results panel that pulls headline metrics out of both embedded apps, so you can compare progress, current pacing, and best-so-far values without constantly scanning both native sidebars. The originating apps were also updated to behave more smoothly in watch mode by using delta-time-aware animation and by limiting how much background training work can happen in a single frame.
