...
## Dashboard version history automation
- The canonical dashboard history file (`dashboard.versions.json`) is generated by `dashboard_versions/sync_dashboard_versions.py`.
- Update `data-dashboard-version` in `index.html` when the live dashboard version changes.
- Archive the outgoing dashboard into `dashboard_versions/<version>/index.html`.
- Update `dashboard_versions/version_notes.json`, then run `python dashboard_versions/sync_dashboard_versions.py`.
- The preview workflow also runs this sync script so `dashboard.versions.json` stays in sync on future pushes.
